...
import LoveSeriesFetch from "../components/FetchSeries/LoveSeriesFetch";
import HorrorSeriesFetch from "../components/FetchSeries/HorrorSeriesFetch";
import ActionSeriesFetch from "../components/FetchSeries/ActionSeriesFetch";
import AnimationSeriesFetch from "../components/FetchSeries/AnimationSeriesFetch";
    ...
    const search = (e) => {
        ...
        if (valueGenre === "") {
            const fetchData = async () => {
                setIsLoading(true);
                const res = await fetch(`https://api.betaseries.com/search/shows?client_id=${process.env.REACT_APP_CLIENT_ID}&text=${value}&limit=50&offset=0`)
                const json = await res.json();
                setSeriesSearch(json.shows)
                setIsLoading(false);
            }
            fetchData();
        } else if (value === "") {
            const fetchData = async () => {
                setIsLoading(true);
                const res = await fetch(`https://api.betaseries.com/search/shows?client_id=${process.env.REACT_APP_CLIENT_ID}&limit=50&offset=0&genres=${valueGenre}`)
                const json = await res.json();
                setSeriesSearch(json.shows)
            ...
            const fetchData = async () => {
                setIsLoading(true);
                const res = await fetch(`https://api.betaseries.com/search/shows?client_id=${process.env.REACT_APP_CLIENT_ID}&text=${value}&limit=50&offset=0&genres=${valueGenre}`)
                const json = await res.json();
                setSeriesSearch(json.shows)
                setIsLoading(false);
            }
        ...
        if (value !== "" || valueGenre !== "") {
            document.getElementById('series_category').classList.add('hidden');
            document.getElementById('series_search').classList.remove('hidden');
        } else {
            document.getElementById('series_search').classList.add('hidden');
            document.getElementById('series_category').classList.remove('hidden');
        }
    }
    return (
        <>
                    ...
                    <form action="" onSubmit={search}>
                        <div className="flex sm:grid">
                            <div className="ml-5 flex-1 sm:ml-2">
                                <select name="genre" className="bg-EO text-white border text-center px-4 py-2 p-1 md:px-2 md:py-1 rounded focus:outline-none focus:shadow-outline" onChange={handleChange}>
            ...
            <div id="series_category" className="mt-5">
                <LoveSeriesFetch />
                <HorrorSeriesFetch />
                <ActionSeriesFetch />
                <AnimationSeriesFetch />
            </div>
            <div id="series_search" className="text-white hidden mt-5">
                <h1 className="ml-5 mb-5">Votre recherche</h1>
                {
                    isLoading === true ? (
                        ...
                        <div className="grid grid-cols-5 sm:grid-cols-3">
                            {
                                seriesSearch?.map((x, i) => (
                                    <div key={i}>
                                        <Link to={{ pathname: `/serie/${x.id}` }} >
                                            <div className="ml-5 mb-5 img_thumb sm:ml-2 sm:mr-2" id="img_series">
                                                {
                                                    x.poster ?
                                                ...
                                                <div id="title_series" className="grid grid-cols-2">
                                                    <div className="ml-2">
