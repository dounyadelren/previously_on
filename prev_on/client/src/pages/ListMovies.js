...
import FavMovies from "../components/ListMovies/FavMovies";
import ToSee from "../components/ListMovies/ToSee";
...
const ListMovies = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            setIsLoading(true);
            const res = await fetch(`https://api.betaseries.com/movies/member?client_id=${process.env.REACT_APP_CLIENT_ID}&id=${user.member.id}&state=1`)
            const json = await res.json();
            setMoviesSeen(json.movies)
            setIsLoading(false);
        }
        fetchData();
    }, [user.member.id])
    ...
    return (
        <>
            ...
            <div id="moviesSeen" className="text-white mt-5">
                <h1 className="ml-5 mb-5">Les films que vous avez vu</h1>
                {
                    isLoading === true ? (
                            ...
                            <Carousel responsive={responsive} autoplay={false} infinite={false} slidesToSlide={1} draggable={true} removeArrowOnDeviceType={["tablet", "mobile"]}>
                                {
                                    moviesSeen?.map((x, i) => (
                                        <div key={i}>
                                            <Link to={{ pathname: `/movie/${x.id}` }} state={{ status: x.user.status }} >
