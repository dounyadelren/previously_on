...
const Series = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            const res = await fetch(`https://api.betaseries.com/shows/list?client_id=${process.env.REACT_APP_CLIENT_ID}&filter=new&platforms&order=popularity`)
            const json = await res.json();
            setSeries(json.shows)
            setIsLoading(false);
        }
        fetchData()
    }, [])
    ...
    return (
        <>
            <h1 className="ml-5 mb-5">Série les plus populaires</h1>
            {
                isLoading === true ? (
                        ...
                        <Carousel responsive={responsive} autoplay={false} infinite={false} slidesToSlide={1} draggable={true} removeArrowOnDeviceType={["tablet", "mobile"]}>
                            {
                                series?.map((res, i) => (
                                    <div key={i}>
                                        <Link to={{ pathname: `/serie/${res.id}` }} >
                                            <div className="ml-5 mb-5 img_thumb" id="img_movies">
                                                {
                                                    res.images.poster ?
                                                        <img src={res.images.poster} className="object-cover sm:h-[50%]" alt="chargement" />
                                                        :
