...
const FavSeries = () => {
    ...
    const [showsFav, setshowsFav] = useState();
    const [isLoading, setIsLoading] = useState(false)

    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            const res = await fetch(`https://api.betaseries.com/shows/favorites?client_id=${process.env.REACT_APP_CLIENT_ID}&id=${user.member.id}`)
            const json = await res.json();
            setshowsFav(json.shows)
            setIsLoading(false);
        }
        fetchData();
    }, [user.member.id])
    ...
    return (
        <div id="moviesFav" className="text-white mt-5">
            <h1 className="ml-5 mb-5">Vos séries favorites</h1>
            {
                isLoading === true ? (
