...
const SeriesArchivees = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            const res = await fetch(`https://api.betaseries.com/shows/favorites?client_id=${process.env.REACT_APP_CLIENT_ID}&id=${user.member.id}&status=archived`)
            const json = await res.json();
            setArchives(json.shows)
            setIsLoading(false);
        }
        fetchData();
    }, [user.member.id])
    ...
    return (
        <div id="moviesFav" className="text-white mt-5">
            <h1 className="ml-5 mb-5">Vos séries archivées</h1>
            {
                isLoading === true ? (
                        ...
                        <Carousel responsive={responsive} autoplay={false} infinite={false} slidesToSlide={1} draggable={true} removeArrowOnDeviceType={["tablet", "mobile"]}>
                            {
                                archives?.map((x, i) => (
                                    <div key={i}>
                                        <Link to={{ pathname: `/serie/${x.id}` }} state={{ myStatus: 1, myShow: 1 }}>
                                            <div className="ml-5 mb-5 img_thumb" id="img_movies">
                                                <img className="object-cover" src={x.images.poster ? x.images.poster : x.show} alt="chargement" />
...
export default SeriesArchivees