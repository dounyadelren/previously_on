...
const HorrorFetch = () => {
    ...
    useEffect(() => {
        if (window.screen.width <= 733) {
            fetch(`https://api.betaseries.com/search/movies?client_id=${process.env.REACT_APP_CLIENT_ID}&limit=6&offset=0&genres=Horror`)
                .then(res => res.json())
                .then(data => setMoviesHorror(data.movies))
        } else {
            fetch(`https://api.betaseries.com/search/movies?client_id=${process.env.REACT_APP_CLIENT_ID}&limit=5&offset=0&genres=Horror`)
                .then(res => res.json())
                .then(data => setMoviesHorror(data.movies))
        }
    }, [])
