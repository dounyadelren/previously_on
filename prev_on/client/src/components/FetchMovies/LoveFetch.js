...
const LoveFetch = () => {
    ...
    useEffect(() => {
        if (window.screen.width <= 733) {
            fetch(`https://api.betaseries.com/search/movies?client_id=${process.env.REACT_APP_CLIENT_ID}&limit=6&offset=0&genres=Romance`)
                .then(res => res.json())
                .then(data => setMoviesRomance(data.movies))
        } else {
            fetch(`https://api.betaseries.com/search/movies?client_id=${process.env.REACT_APP_CLIENT_ID}&limit=5&offset=0&genres=Romance`)
                .then(res => res.json())
                .then(data => setMoviesRomance(data.movies))
        }
    }, [])
    ...
    return (
        <>
            <div className="text-white">
                <h1 className="ml-5 mb-5">Romance</h1>
                <div className="grid grid-cols-5 gap-1 sm:grid sm:grid-cols-3">
                    {
                        moviesRomance?.map((x, i) => (
                            <div key={i}>
