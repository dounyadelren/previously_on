...
const FavMovies = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            const res = await fetch(`https://api.betaseries.com/movies/favorites?client_id=${process.env.REACT_APP_CLIENT_ID}&id=${user.member.id}`)
            const json = await res.json();
            setMoviesFav(json.movies)
            setIsLoading(false);
        }
        fetchData();
    }, [user.member.id])
    ...
    return (
        <>
            <div id="moviesFav" className="text-white mt-5">
                ...
                <h1 className="ml-5 mb-5">Vos films favoris</h1>
                {
                    isLoading === true ? (
                            ...
                            <Carousel responsive={responsive} autoplay={false} infinite={false} slidesToSlide={1} draggable={true} removeArrowOnDeviceType={["tablet", "mobile"]}>
                                {
                                    moviesFav?.map((x, i) => (
                                        <div key={i}>
                                            <Link to={{ pathname: `/movie/${x.id}` }} state={{ status: x.user.favorited }}>
