...
const ToSee = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            const res = await fetch(`https://api.betaseries.com/movies/member?client_id=${process.env.REACT_APP_CLIENT_ID}&id=${user.member.id}&state=0`)
            const json = await res.json();
            setMoviesToSee(json.movies)
            setIsLoading(false);
        }
        fetchData();
    }, [user.member.id])
    ...
    return (
        <>
            <div id="moviesFav" className="text-white mt-5">
                <h1 className="ml-5 mb-5">Vos films à voir</h1>
                {
                    isLoading === true ? (
