...
const LoveSeriesFetch = () => {
    const [seriesRomance, setSeriesRomance] = useState();

    useEffect(() => {
        if (window.screen.width <= 734) {
            fetch(`https://api.betaseries.com/search/shows?client_id=${process.env.REACT_APP_CLIENT_ID}&limit=6&offset=0&genres=Romance`)
                .then(res => res.json())
                .then(data => setSeriesRomance(data.shows))
        } else {
            fetch(`https://api.betaseries.com/search/shows?client_id=${process.env.REACT_APP_CLIENT_ID}&limit=5&offset=0&genres=Romance`)
                .then(res => res.json())
                .then(data => setSeriesRomance(data.shows))
        }
    }, [])
...
export default LoveSeriesFetch